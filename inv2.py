...
class MainUI(tk.Frame):
    def __init__(self, master):
        ...
        res = get_cnt('chars', dest=None)
        for toon in res:
            if toon['league'] not in self.leagues:
                self.leagues[toon['league']] = {}
            ldict = self.leagues[toon['league']]
            ldict[toon['name']] = toon
            self.toons.append(toon)
        super().__init__(master)
        for league in self.leagues:
            wdgt = self.league_toons(league)
            wdgt.grid(sticky=tk.EW)
        self.info = tk.Text(self, background='white', width=50)
        self.info.grid(column=1, row=0, rowspan=len(self.leagues), sticky=tk.NSEW)
        self.columnconfigure(0, weight=1)
        self.columnconfigure(1, weight=1)
        self.grid()

    def league_toons(self, league):
        blg = tk.LabelFrame(self, text=league)
        for t_name, toon in self.leagues[league].items():
            twdgt = tk.Button(blg, text=t_name,
                              command= lambda t=toon: self.show_toon(t))
            twdgt.grid(sticky=tk.EW)
        blg.columnconfigure(0, weight=1)
        return blg

    def show_toon(self, toon):
        self.info.insert(tk.END, 'showing ' + toon['name'])
